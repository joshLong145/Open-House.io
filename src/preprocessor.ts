...
export function generateModel(data: any[]): BaseTransformModel[] { 
    ...
    for (const entry of data) {
        const modelDef: BaseTransformModel = new BaseTransformModel(entry, entry.limit, entry.Cookie, entry.price_upper);
...
export function parseData(configPath: string): any {
    try {
...
export function resolveModelTransform(models: BaseTransformModel[]) {
